...
#include <fstream>
#include <sstream>
#define SourcePort "42100"
#define DestPORT 44100
#define LH "127.0.0.1"
using namespace std;

int main(){
//Parse the literature.txt and put the (book, num) pairs into a map
//The usage of getline() is referred from a stockoverflow post
	map<string, string> booklist;
	ifstream in("literature.txt");
	string line;
...
//end of copy
	...
	// finish setup phase
	cout<<"Server L is up and running using UDP on port "<<SourcePort<<endl;
	...
	while (1){
		...
		cout<<"Server L received "<<msg<<" code from the Main Server "<<endl;
		...
		if (it!=booklist.end()){
			...
			cout<<"Server L finished sending the availability status of code "<<msg<<" to the Main Server using UDP on port "<<SourcePort<<endl;
			}
			else{
