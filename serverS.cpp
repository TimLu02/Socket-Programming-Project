...
#include <fstream>
#include <sstream>
#define SourcePort "41100"
#define DestPORT 44100
#define LH "127.0.0.1"
using namespace std;

int main(){
//Parse the science.txt and put the (book, num) pairs into a map
//The usage of getline() is referred from a stockoverflow post
	map<string, string> booklist;
	ifstream in("science.txt");
	string line;
...
//end of copy
	...
	// finish setup phase
	cout<<"Server S is up and running using UDP on port "<<SourcePort<<endl;
	...
	while (1){
		...
		cout<<"Server S received "<<msg<<" code from the Main Server "<<endl;
		map<string,string>::iterator it = booklist.find((string)msg);
		if (it!=booklist.end()){

			if (it->second !="0"){
				char* bn = new char[(it->second).length()];
				strcpy(bn,(it->second).c_str());
				cout<<bn<<endl;
				if((numbytes = sendto(sockfd,bn,sizeof(bn),0,(struct sockaddr*)&dest,sizeof(dest)))==-1){
					perror("talker: sendto");
					exit(1);
				}
			cout<<"Server S finished sending the availability status of code "<<msg<<" to the Main Server using UDP on port "<<SourcePort<<endl;
			}
			else{
